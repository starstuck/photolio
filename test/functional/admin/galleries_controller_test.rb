...
class Admin::GalleriesControllerTest < ActionController::TestCase
  def test_should_get_index
    get :index, :site_id => sites(:studio).id
    assert_response :success
    assert_not_nil assigns(:galleries)
  end

  def test_should_get_new
    get :new, :site_id => sites(:studio).id 
    assert_response :success
  end
  ...
  def test_should_show_gallery
    get :show, :site_id => sites(:studio).id, :id => galleries(:one).id
    assert_response :success
  end

  def test_should_get_edit
    get :edit, :site_id => sites(:studio).id, :id => galleries(:one).id
    assert_response :success
  end
  ...
  def test_should_destroy_gallery
    assert_difference('Gallery.count', -1) do
      delete :destroy, :site_id => sites(:studio), :id => galleries(:one).id
    end
