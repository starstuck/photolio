...
class Admin::TopicsControllerTest < ActionController::TestCase
  def test_should_get_index
    get :index, :site_id => sites(:studio)
    assert_response :success
    assert_not_nil assigns(:topics)
  end

  def test_should_get_new
    get :new, :site_id => sites(:studio)
    assert_response :success
  end
  ...
  def test_should_show_topic
    get :show, :site_id => sites(:studio), :id => topics(:one).id
    assert_response :success
  end

  def test_should_get_edit
    get :edit, :site_id => sites(:studio), :id => topics(:one).id
    assert_response :success
  end

  def test_should_update_topic
    put :update, :site_id => sites(:studio), :id => topics(:one).id, :topic => { :title => 'Updated title' }
    assert_redirected_to admin_site_topic_path(assigns(:site), assigns(:topic))
  end

  def test_should_destroy_topic
    assert_difference('Topic.count', -1) do
      delete :destroy, :site_id => sites(:studio), :id => topics(:one).id
    end
